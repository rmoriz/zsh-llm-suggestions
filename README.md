...
bindkey '^r' zsh_llm_suggestions_openrouter # Ctrl + R to have OpenRouter suggest a command given a English description
...
For `zsh_llm_suggestions_openrouter` (OpenRouter suggestions):
- Set the `OPENROUTER_API_KEY` environment variable to your API key. You can get it
  from [https://openrouter.ai/keys](https://openrouter.ai/keys). Note
  that every suggestion costs a small amount of money, you are solely responsible for
  these charges.
  ```
  export OPENROUTER_API_KEY="..."
  ```
- Optional: Set the `OPENROUTER_BASE_URL` environment variable (defaults to `https://openrouter.ai/api/v1`).
  ```
  export OPENROUTER_BASE_URL="https://openrouter.ai/api/v1"
  ```
- Optional: Set the `OPENROUTER_MODEL` environment variable to choose a different model (defaults to `anthropic/claude-3-haiku:beta`).
  ```
...
Type out what you'd like to do in English, then hit ctrl+P, ctrl+O, or ctrl+R (or whatever hotkey)
you configured. `zsh-llm-suggestions` will then query OpenAI, GitHub Copilot, or OpenRouter, and replace
...
ctrl+alt+O to have OpenAI explain the command in English, ctrl+alt+P to have
GitHub Copilot explain it, or ctrl+alt+R to have OpenRouter explain it.
...
3. OpenRouter. Requires an OpenRouter API key. Supports various models via environment variables.
